...
# print (objs)
# for item in c.last_event.metadata["objects"]:
#     print(item, '\n\n')

#print(c.last_event.metadata["objects"][0].keys())
...
def action_queue(act):
    ...
    try:
        ...
        if act['action'] == 'ObjectNavExpertAction':
            ...
            if multi_agent_event.metadata['errorMessage'] != "":
                print (multi_agent_event.metadata['errorMessage'])
                sys.exit(0)
                print ("Putting down the object FAILED. Dropping the object to empty the agent hand.")
    ...
    #     if 'Lettuce' in object['objectType']:
    #         object['rotation'] = {'x': 0, 'y': -90, 'z': -90}
...
def PickupObject(agents, pick_obj):
    ...
    for idx in range(num_agents):
        ...
        action_queue({'action':'PickupObject', 'objectId':pick_obj_id, 'agent_id':agent_id})
        time.sleep(1)
...
def PutObject(agent, put_obj, recp):
    # if not stop_signal.is_set():
        ...
        action_queue({'action':'PutObject', 'objectId':recp_obj_id, 'agent_id':agent_id})
        time.sleep(1)
        ...
        if "StoveKnob" in sw_obj:
            for obj in objs:
                ...
                if match is not None:
                    ...
                    # time.sleep(1)
                    action_queue({'action':'ToggleObjectOn', 'objectId':sw_obj_id, 'agent_id':agent_id})
                    time.sleep(0.5)
        ...
        if "StoveKnob" in sw_obj:
            for obj in objs:
                ...
                if match is not None:
                    sw_obj_id = obj
                    GoToObject(agent, sw_obj_id)
                    action_queue({'action':'ToggleObjectOff', 'objectId':sw_obj_id, 'agent_id':agent_id})
                    time.sleep(0.5)
...
def OpenObject(agent, opn_obj):
    # if not stop_signal.is_set():
        ...
        # time.sleep(1)
        action_queue({'action':'OpenObject', 'objectId':opn_obj_id, 'agent_id':agent_id})
        time.sleep(1)
...
def CloseObject(agent, clz_obj):
    # if not stop_signal.is_set():
        ...
        # time.sleep(1)
        action_queue({'action':'CloseObject', 'objectId':clz_obj_id, 'agent_id':agent_id}) 
...
def BreakObject(agent, brk_obj):
    # if not stop_signal.is_set():
        ...
        # time.sleep(1)
        action_queue({'action':'BreakObject', 'objectId':brk_obj_id, 'agent_id':agent_id}) 
        time.sleep(1)
...
def SliceObject(agent, slc_obj, tool_obj):
    # if not stop_signal.is_set():
        ...
        action_queue({'action':'SliceObject', 'objectId':slc_obj_id, 'agent_id':agent_id})      
        time.sleep(1)
        print ("Performing Action Slice ", slc_obj_id, ", ", agent_name, agent_type)
        action_queue({'action':'DropHandObject', 'agent_id':agent_id}) 
        time.sleep(1)
...
def CleanObject(agent, cln_obj, tool_obj, clnsr_obj=None):
    # if not stop_signal.is_set():
        ...
        # time.sleep(1)
        action_queue({'action':'CleanObject', 'objectId':cln_obj_id, 'agent_id':agent_id}) 
        time.sleep(1)
...
def UseUpObject(agent, use_obj):
    # if not stop_signal.is_set():
        ...
        # time.sleep(1)
        action_queue({'action':'UseUpObject', 'objectId':use_obj_id, 'agent_id':agent_id}) 
        time.sleep(1)
...
def ThrowObject(agent, thrw_obj):
    # if not stop_signal.is_set():
        ...
        action_queue({'action':'ThrowObject', 'agent_id':agent_id}) 
        time.sleep(1)
        print ("Performing Action Throw ", thrw_obj_id, ", ", agent_name, agent_type)

def prepare_toast(agents):
    ### SubTask 1: Slice the bread and toast it using the toaster. ###
    # Slice the bread.
    SliceObject(agents[0], 'Bread|-00.18|+01.12|+01.89', 'Knife|-01.48|+00.93|+00.42')
    # Put the sliced bread in the toaster.
    PutObject(agents[0], 'BreadSliced', 'Toaster|-00.23|+01.04|+02.87')
    # Switch on the toaster to toast the bread.
    SwitchOn(agents[0], 'Toaster|-00.23|+01.04|+02.87')
    # Wait for the bread to toast.
    time.sleep(5)
    # Switch off the toaster.
    SwitchOff(agents[0], 'Toaster|-00.23|+01.04|+02.87')

def scramble_eggs(agents):
    ### SubTask 2: Break the eggs into the pan and scramble them on the stove burner. ###
    # Open the fridge to access the eggs.
    # OpenObject(agents[0], 'Fridge|-03.52|+00.00|+02.72')
    # Retrieve the egg and put it in the pan.
    # PutObject(agents[0], 'Egg|-03.37|+01.30|+02.85', 'Pan|-00.27|+01.04|+02.56')
    # Close the fridge.
    # CloseObject(agents[0], 'Fridge|-03.52|+00.00|+02.72')
    # Break the egg.
    # BreakObject(agents[0], 'Egg|-03.37|+01.30|+02.85')
    # Place the pan on an empty StoveBurner.
    PutObject(agents[0], 'Pan|-00.27|+01.04|+02.56', 'StoveBurner|-03.48|+01.10|+00.48')
    # Switch on the StoveBurner.
    # SwitchOn(agents[0], 'StoveKnob')
    # Wait for the eggs to scramble.
    time.sleep(5)
    # Switch off the StoveBurner.
    # SwitchOff(agents[0], 'StoveKnob')

def slice_tomato(agents):
    ### SubTask 3: Slice the tomato using the knife. ###
    # Slice the tomato.
    SliceObject(agents[0], 'Tomato|-00.75|+01.08|+02.40', 'Knife|-01.48|+00.93|+00.42')

def serve_meal(agents):
    ### SubTask 4: Serve the scrambled eggs, toast, and sliced tomato on a plate. ###
    # Place the plate on the dining table.
    # PutObject(agents[0], 'Plate|-01.19|+00.92|+00.40', 'DiningTable|-00.62|+00.02|+02.49')
    # Place the scrambled eggs on the plate.
    # PutObject(agents[0], 'EggCracked', 'Plate|-01.19|+00.92|+00.40')
    # Place the toasted bread on the plate.
    PutObject(agents[0], 'BreadSliced', 'Plate|-01.19|+00.92|+00.40')
    # Place the sliced tomato on the plate.
    PutObject(agents[0], 'TomatoSliced', 'Plate|-01.19|+00.92|+00.40')

# run the functions
agents = [{'name': 'agent1', 'type': 'robot'}]
# prepare_toast(agents)
scramble_eggs(agents)
slice_tomato(agents)
# serve_meal(agents)
needed_objects = ['Bread', 'BreadSliced', 'Toaster', 'Egg', 'EggCracked', 'Pan', 'StoveBurner', 'Tomato', 'TomatoSliced', 'Knife', 'Plate']
for i in range(25):
