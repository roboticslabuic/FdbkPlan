...
def single_agent_code_prep(decomposed_plan_code, gpt_version):
    ...
    if "gpt" not in gpt_version:
        ...
        _, text = LM(messages_single_agent, gpt_version, max_tokens=1000, frequency_penalty=0.0)
...
def recursive_task_execution(prompt, messages, single_agent_plan, gpt_version, decomp_freq_penalty, decomposed_plan_code):
    ...
    try:
        ...
        if user_input.lower() == "yes":
            ...
            if "gpt" not in gpt_version:
                _, text = LM(prompt, gpt_version, max_tokens=1000, stop=["def"], frequency_penalty=decomp_freq_penalty)
            else:
                messages.append({"role": "assistant", "content": decomposed_plan_code})
                messages.append({"role": "user", "content": additional_instructions})
                _, text = LM(messages, gpt_version, max_tokens=1000, frequency_penalty=decomp_freq_penalty) #1600 for gpt4o
            ...
            while user_input.lower() != "yes":
                ...
                if "gpt" not in gpt_version:
                    ...
                    messages.append({"role": "user", "content": additional_instructions})
                    _, text = LM(messages, args.gpt_version, max_tokens=1000, frequency_penalty=decomp_freq_penalty) #1600 for gpt4o
...
if __name__ == "__main__":
    ...
    obj_extract_prompt_file = open(f"./prompt_examples/{gpt_folder}/floorplan_{floor_plan_type}/gpt/{args.prompt_task_desc_obj_extraction}" + ".txt", "r")
    ...
    decompose_prompt_file = open(f"./prompt_examples/{gpt_folder}/floorplan_{floor_plan_type}/gpt/{args.prompt_decompse_set}" + ".txt", "r")
    ...
    states = get_objs_state(extracted_objs_list, args.floor_plan)
    context = get_objs_context(extracted_objs_list)
    decompose_code_prompt_file = open(f"./prompt_examples/{gpt_folder}/floorplan_{floor_plan_type}/gpt/{args.prompt_decompse_code_set}" + ".txt", "r")
    ...
    if "gpt" not in args.gpt_version:
        ...
        _, text = LM(messages, args.gpt_version, max_tokens=1000, frequency_penalty=decomp_code_freq_penalty)   
    ...
    allocate_prompt_file = open(f"./prompt_examples/{gpt_folder}/floorplan_{floor_plan_type}/gpt/{args.prompt_allocation_set}" + ".txt", "r")
    ...
    if "gpt" not in args.gpt_version:
        ...
        _, text = LM(messages,args.gpt_version, max_tokens=1000, frequency_penalty=0.0) #0.30 #1800 for gpt4o
    ...
    while user_input.lower() != "yes":
        ...
        if "gpt" not in args.gpt_version:
            ...
            messages.append({"role": "assistant", "content": text})
            messages.append({"role": "user", "content": additional_instructions})
            _, text = LM(messages, args.gpt_version, max_tokens=1000, frequency_penalty=0.0)
